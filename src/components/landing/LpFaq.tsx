    ...
    <div className="grid gap-3">
      {items.map((item) => (
        <details
          key={item.q}
          name="lp-faq"
          className="group rounded-3xl bg-white ring-1 ring-brand-100 transition open:ring-brand-300 hover:ring-brand-200"
        >
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-5 [&::-webkit-details-marker]:hidden">
            <span className="font-display text-[1.08rem] font-semibold text-brand-950">
              {item.q}
            </span>
          ...
          <p className="px-6 pb-6 text-[1.02rem] leading-relaxed text-slate-600">{item.a}</p>
        </details>
      ))}
    </div>
  );
